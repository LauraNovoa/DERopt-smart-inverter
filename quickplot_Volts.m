...
%%  %Voltage Profile for all timesteps - DLPF vs. ACPF
    ...
    %ACPF
    h1 = plot(1:n,BusVolAC(:,2:end),'Color',[pallete(2,:) 1],'LineWidth',0.6);hold on;
    
    %Linearized
    if exist('Volts','var') 
        if dlpfc
           h2 = plot(1:n,Volts(:,2:end),'Color',[pallete(1,:) 0.2 ],'LineWidth',0.6);hold on;
        else 
           h2 = plot(1:n,Volts(:,2:end),'Color',pallete(7,:),'LineWidth',0.6);hold on;
        end
...
%%  Voltage Profile for all timesteps - only for DLPF
    ...
    if exist('Volts','var') 
        ...
        h2 = plot(1:n,Volts(:,2:end),'Color',pallete(2,:),'LineWidth',0.6);hold on;
        
%         if dlpfc
%            h2 = plot(1:n,Volts(:,2:end),'Color',[pallete(1,:) 0.5 ],'LineWidth',1.5);hold on;
%         else 
%            h2 = plot(1:n,Volts(:,2:end),'Color',pallete(7,:),'LineWidth',0.6);hold on;
%         end
...
%     text(nodeov+d, ov+d, sprintf('%.3f @ node %.0f',ov,nodeov),'FontSize', 11)
    ...
    %Linearized OV and UV Markers   
         p3 = plot(nodeuvL, uvL,'o' ,'MarkerSize', 5,'Color','k','MarkerFaceColor',pallete(2,:));
         p4 = plot(nodeovL, ovL,'o' ,'MarkerSize', 5,'Color','k','MarkerFaceColor',pallete(2,:));
...
%     
    ...
    [leg, objects] = legend([h2(1) h4],'Linearized (DLPF)',sprintf('Voltage at hour %.0d',ts),'Location','northwest');
    ...
    h2 = plot(1:n,BusVolAC(:,2:end),'Color',pallete(2,:),'LineWidth',0.6);hold on;
