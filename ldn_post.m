...
TranformerResults = table(max(TloadkVA(:,T_map))',max(TloadkVAR(:,T_map))', T_PV(T_map) , T_EES(T_map), T_REES(T_map), T_rated(T_map)', max(Sinj(:,T_map))',min(Sinj(:,T_map))','VariableNames', {'T_load_max_kVA', 'T_load_max_kVAR' ,'T_PV_kW', 'T_EES_kWh', 'T_RESS_kWh', 'T_rated_kVA','Sinj_max_kVA', 'Sinj_min_kVA'})
...
adopt = [pv_adopt' , ees_adopt', rees_adopt', inv_adopt'];
